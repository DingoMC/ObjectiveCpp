...
int main() {
    ...
    for (int i = 0; i<elements.size();i++) name_indexx.modify(elements[i], boost::bind(AlaToAlicja,_1));
    ...
    for (name_type2::iterator it2 = iterStart.first; it2 != iterStop.second; ++it2) it2->show();
    auto from = name_indexx2.lower_bound("Ala");
    auto to = name_indexx2.upper_bound("Piotr");
    cout <<"przedzial od Ala do Piotr - wersja 2"<<endl;
    for (auto it2 = from; it2 != to; ++it2) it2->show();
    const auto &rand_indexx = persons2.get<2>();
